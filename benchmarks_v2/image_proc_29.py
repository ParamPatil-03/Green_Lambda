...
def lambda_handler(event, context):
    try:
        ...
        pixels = [[float((r * c) % 256) for c in range(580)] for r in range(580)]
        
        filtered = [[0.0 for _ in range(580)] for _ in range(580)]
        kernel_sum = 0.0
        
        for loop in range(10):
            for r in range(1, 580 - 1):
                for c in range(1, 580 - 1):
                    val = (
        ...
        resized = []
        for r in range(0, 580, 2):
            row = []
            for c in range(0, 580, 2):
