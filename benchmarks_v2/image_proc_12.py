...
def lambda_handler(event, context):
    try:
        ...
        pixels = [[float((r * c) % 256) for c in range(144)] for r in range(144)]
        
        filtered = [[0.0 for _ in range(144)] for _ in range(144)]
        kernel_sum = 0.0
        
        for loop in range(1):
            for r in range(1, 144 - 1):
                for c in range(1, 144 - 1):
                    val = (
        ...
        resized = []
        for r in range(0, 144, 2):
            row = []
            for c in range(0, 144, 2):
