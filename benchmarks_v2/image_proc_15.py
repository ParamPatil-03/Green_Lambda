...
def lambda_handler(event, context):
    try:
        ...
        pixels = [[float((r * c) % 256) for c in range(180)] for r in range(180)]
        
        filtered = [[0.0 for _ in range(180)] for _ in range(180)]
        kernel_sum = 0.0
        
        for loop in range(1):
            for r in range(1, 180 - 1):
                for c in range(1, 180 - 1):
                    val = (
