...
def lambda_handler(event, context):
    try:
        ...
        pixels = [[float((r * c) % 256) for c in range(108)] for r in range(108)]
        
        filtered = [[0.0 for _ in range(108)] for _ in range(108)]
        kernel_sum = 0.0
        
        for loop in range(1):
            for r in range(1, 108 - 1):
                for c in range(1, 108 - 1):
                    val = (
        ...
        resized = []
        for r in range(0, 108, 2):
            row = []
            for c in range(0, 108, 2):
