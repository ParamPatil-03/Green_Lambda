...
def lambda_handler(event, context):
    try:
        ...
        pixels = [[float((r * c) % 256) for c in range(228)] for r in range(228)]
        
        filtered = [[0.0 for _ in range(228)] for _ in range(228)]
        kernel_sum = 0.0
        
        for loop in range(1):
            for r in range(1, 228 - 1):
                for c in range(1, 228 - 1):
                    val = (
        ...
        resized = []
        for r in range(0, 228, 2):
            row = []
            for c in range(0, 228, 2):
