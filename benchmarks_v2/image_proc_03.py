...
def lambda_handler(event, context):
    try:
        ...
        pixels = [[float((r * c) % 256) for c in range(36)] for r in range(36)]
        
        filtered = [[0.0 for _ in range(36)] for _ in range(36)]
        kernel_sum = 0.0
        
        for loop in range(1):
            for r in range(1, 36 - 1):
        ...
        resized = []
        for r in range(0, 36, 2):
            row = []
            for c in range(0, 36, 2):
