...
def lambda_handler(event, context):
    try:
        ...
        pixels = [[float((r * c) % 256) for c in range(252)] for r in range(252)]
        
        filtered = [[0.0 for _ in range(252)] for _ in range(252)]
        kernel_sum = 0.0
        
        for loop in range(1):
            for r in range(1, 252 - 1):
                for c in range(1, 252 - 1):
                    val = (
        ...
        resized = []
        for r in range(0, 252, 2):
            row = []
            for c in range(0, 252, 2):
