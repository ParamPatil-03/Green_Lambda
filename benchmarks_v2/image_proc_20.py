...
def lambda_handler(event, context):
    try:
        ...
        pixels = [[float((r * c) % 256) for c in range(240)] for r in range(240)]
        
        filtered = [[0.0 for _ in range(240)] for _ in range(240)]
        kernel_sum = 0.0
        
        for loop in range(1):
            for r in range(1, 240 - 1):
                for c in range(1, 240 - 1):
                    val = (
